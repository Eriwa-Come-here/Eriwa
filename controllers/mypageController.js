...
module.exports={
    ...
    recommend: async (req, res, next) => {
        try {
            ...
            let query1 = "SELECT `user`.*, `post`.*, COUNT(`recommend`.`user_id` ) AS `recommend_count` FROM `recommend` LEFT JOIN `post` ON `recommend`.`post_id` = `post`.`post_id` LEFT JOIN `user` ON `post`.`user_id` = `user`.`user_id` WHERE `recommend`.`user_id`=? GROUP BY `recommend`.`user_id`, `recommend`.`post_id` ORDER BY `written_date` DESC";
            let query2 = "SELECT `recommend`.*, COUNT(`comment`.`user_id`) AS `comment_count` FROM `recommend` LEFT JOIN `comment` ON `recommend`.`post_id` = `comment`.`post_id` LEFT JOIN `post` ON `recommend`.`post_id` = `post`.`post_id` WHERE `recommend`.`user_id`=? GROUP BY `recommend`.`user_id`, `recommend`.`post_id` ORDER BY `post`.`written_date` DESC";
    ...
    recommendPlace: async (req, res, next) => {
        try {
            ...
            let query1 = "SELECT `user`.*, `post`.*, COUNT(`recommend`.`user_id` ) AS `recommend_count` FROM `recommend` LEFT JOIN `post` ON `recommend`.`post_id` = `post`.`post_id` LEFT JOIN `user` ON `post`.`user_id` = `user`.`user_id` WHERE `recommend`.`user_id`=? AND `post`.`address1` = ? GROUP BY `recommend`.`user_id`, `recommend`.`post_id` ORDER BY `written_date` DESC";
