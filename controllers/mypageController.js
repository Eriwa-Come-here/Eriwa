...
module.exports={
    ...
    recommend: async (req, res, next) => {
        try {
            ...
            let query1 = "SELECT `user`.*, `post`.*, `post`.`nickname` AS `writer`, COUNT(`recommend`.`user_id` ) AS `recommend_count` FROM `recommend` LEFT JOIN `post` ON `recommend`.`post_id` = `post`.`post_id` LEFT JOIN `user` ON `recommend`.`user_id` = `user`.`user_id` WHERE `recommend`.`user_id`=? GROUP BY `recommend`.`user_id`, `recommend`.`post_id` ORDER BY `written_date` DESC";
