...
module.exports={
    ...
    index : async (req, res, next) => {
        try {
            ...
            let query3 = "SELECT `user`.*, `post`.*, COUNT(`comment`.`post_id`) AS `comment_count` FROM `post` LEFT JOIN `comment` ON `post`.`post_id` = `comment`.`post_id` LEFT JOIN `user` ON `user`.`user_id` = `post`.`user_id` WHERE `post`.`view_count` = (SELECT MAX(`view_count`) FROM `post` GROUP BY `post`.`post_id` LIMIT 1) AND DATE(`post`.`written_date`) between '"+search.sliceDate(1)+"' AND '"+currentDate.currentDate()+"' GROUP BY `post`.`post_id` LIMIT 1";
